...
impl ExpenseService {
    ...
    pub async fn analyze_expenses_by_category(
        ...
        for expense in &expenses {
            *category_totals.entry(expense.category.clone()).or_insert(Decimal::ZERO) += expense.amount;
            
            let month_key = expense.date.format("%Y-%m").to_string();
            *monthly_totals.entry(month_key).or_insert(Decimal::ZERO) += expense.amount;
        }
        ...
        let average_expense = if expenses.is_empty() {
            ...
            total_spent / Decimal::from(expenses.len() as u64)
        };
    ...
    fn get_top_merchants(&self, expenses: &[Expense]) -> Vec<(String, Decimal)> {
        let mut merchant_totals = HashMap::new();
        
        for expense in expenses {
            if let Some(ref merchant) = expense.merchant {
                *merchant_totals.entry(merchant.clone()).or_insert(Decimal::ZERO) += expense.amount;
            }
        }

        let mut merchants: Vec<(String, Decimal)> = merchant_totals.into_iter().collect();
        merchants.sort_by(|a, b| b.1.cmp(&a.1));
...
pub struct ExpenseAnalysis {
    ...
    pub monthly_breakdown: HashMap<String, Decimal>,
    pub top_merchants: Vec<(String, Decimal)>,
}
