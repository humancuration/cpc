...
impl BudgetTemplateService {
    ...
    pub fn apply_template(&self, sheet: &Sheet, user_id: Uuid, user_preferences: &dyn packages::domains::finance::application::user_preferences::UserPreferences) -> Result<(), String> {
        ...
        match template_type.template_type {
            TemplateType::MonthlyBudget => self.process_monthly_budget_template(sheet, user_id, currency.clone()),
            TemplateType::WeeklyBudget => self.process_weekly_budget_template(sheet, user_id, currency.clone()),
            TemplateType::ProjectBudget => self.process_project_budget_template(sheet, user_id, currency.clone()),
    ...
    fn process_monthly_budget_template(&self, sheet: &Sheet, user_id: Uuid, currency: packages::domains::finance::domain::primitives::Currency) -> Result<(), String> {
    ...
    fn process_weekly_budget_template(&self, sheet: &Sheet, user_id: Uuid, currency: packages::domains::finance::domain::primitives::Currency) -> Result<(), String> {
    ...
    fn process_project_budget_template(&self, sheet: &Sheet, user_id: Uuid, currency: packages::domains::finance::domain::primitives::Currency) -> Result<(), String> {
        // Project budget template processing
        // This would be similar to monthly but with project-specific logic
        self.process_custom_template(sheet, user_id, currency)
    }
    
    fn process_custom_template(&self, sheet: &Sheet, user_id: Uuid, currency: packages::domains::finance::domain::primitives::Currency) -> Result<(), String> {
        // Custom template processing
        // For now, we'll treat it as a monthly budget
        self.process_monthly_budget_template(sheet, user_id, currency)
    }
