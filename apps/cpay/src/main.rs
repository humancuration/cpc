...
impl cpc_wallet::application::WalletRepository for MockWalletRepository {
    ...
    async fn find_wallet_by_user_id(&self, _user_id: uuid::Uuid) -> Result<Option<cpc_wallet::Wallet>, cpc_wallet::domain::primitives::FinancialError> {
        Ok(None)
    }
    ...
    async fn find_transactions_by_wallet_id(&self, _wallet_id: uuid::Uuid) -> Result<Vec<cpc_wallet::WalletTransaction>, cpc_wallet::domain::primitives::FinancialError> {
        Ok(vec![])
    }
}
