//! Cause feedback collector component for impact visualizations
//!
//! This component provides a reusable feedback collection interface that can be
//! embedded in any cause impact visualization to collect user feedback.
...
/// Properties for the CauseFeedbackCollector component
#[derive(Properties, PartialEq)]
pub struct CauseFeedbackCollectorProps {
    /// Unique identifier for the visualization component
    pub component_id: String,
    
    /// Callback function to handle feedback submission
    #[prop_or_default]
    pub on_feedback_submit: Callback<CauseFeedbackData>,
}

/// Data structure for cause feedback submission
#[derive(Clone, Debug)]
pub struct CauseFeedbackData {
    ...
    /// How the visualization affected cause decisions
    pub decision_impact: Option<String>,
    
    /// Understanding improvement rating (1-10)
    pub understanding_improvement: Option<u8>,
    
    /// Confidence improvement rating (1-10)
    pub confidence_improvement: Option<u8>,
}

/// Cause feedback collector component for impact visualizations
#[styled_component(CauseFeedbackCollector)]
pub fn cause_feedback_collector(props: &CauseFeedbackCollectorProps) -> Html {
    ...
    let understanding = use_state(|| 5u8); // Default to middle value
    let confidence = use_state(|| 5u8); // Default to middle value
    ...
    let on_confidence_change = {
        ...
        Callback::from(move |e: Event| {
            let input: web_sys::HtmlInputElement = e.target_unchecked_into();
            if let Ok(value) = input.value().parse::<u8>() {
                confidence.set(value.clamp(1, 10));
            }
        })
    };
    ...
    let on_submit = {
        ...
        Callback::from(move |_| {
            let feedback_data = CauseFeedbackData {
                ...
                understanding_improvement: Some(*understanding),
                confidence_improvement: Some(*confidence),
            };
    ...
    html! {
        <div class={container_style}>
            <h4 class={title_style}>{"Was this cause visualization helpful for understanding the impact of your contributions?"}</h4>
            ...
            if helpful.is_some() {
                <div>
                    ...
                    <div class={slider_style.clone()}>
                        <div class={slider_label_style.clone()}>
                            <span>{"How much did this visualization improve your understanding of the cause? (1-10)"}</span>
                        ...
                        <input
                            ...
                            min="1"
                            max="10"
                            value={understanding.to_string()}
                            oninput={on_understanding_change}
                            class={slider_input_style.clone()}
                        />
                    </div>
                    
                    <div class={slider_style.clone()}>
                        <div class={slider_label_style.clone()}>
                            <span>{"How much did this visualization increase your confidence in your contribution decisions? (1-10)"}</span>
                    ...
                    <textarea
                        ...
                        placeholder="How could this visualization be improved to better show cause impact?"
                        value={(*comment).clone()}
                        oninput={on_comment_input}
                    />
                    
                    <h4 class={title_style.clone()}>{"How did this visualization affect your contribution decisions? (optional)"}</h4>
                    <textarea
                        class={comment_style}
                        placeholder="Did this visualization help you choose which causes to support or understand your impact?"
